...
class Material_Manager(object):
    ...
    def Load_Material(self, material):
        if material:
            ...
            cwd = os.getcwd()
            full_path = cwd +"\\"+"Default Materials"+"\\"+filename
            try:
    ...
    def Load_Default_Materials(self):
        root = tk.Tk()
        root.withdraw()
        filename = filedialog.askopenfilename(initialdir = os.getcwd(), title = "Load Materials List", filetypes = (("Recipe Materials", "*.rcpm*"),  ("All files", "*.*")))
        try:
    ...
    def Save_Default_Materials(self, ST):
        root = tk.Tk()
        root.withdraw()
        save_dir = os.getcwd() + "\\" +"Default Materials" + "\\"
    ...
    def Initialize_Materials(self):
        ...
        filename = os.getcwd() + "\\" + "Default Materials" + "\\" + "default_materials.rcpm"
        try:
            with open(filename, 'r') as file:
                dict = json.load(file)
                self.full_info = dict
